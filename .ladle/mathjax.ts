...
const typesetMath = ((windowImpl: Window = window) => {
  ...
  return promise = promise.then(() => _typesetMath(windowImpl))
})

const _typesetMath = (windowImpl: Window = window) => {
  ...
  return mathjax.typesetPromise([document, nodes]).then(() => {
    ...
    nodes = nodes.concat(Array.from(document.querySelectorAll(MATH_ML_SELECTOR)))
